...
@pytest.mark.parametrize(
    "key",
    [
        "vpn_client_bytes_received",
        "vpn_client_bytes_sent",
        "vpn_client_packets_received",
        "vpn_client_packets_sent",
        "vpn_client_remote_ip",
        "vpn_client_connected_since",
    ],
)
def test_vpn_client_sensor_translations(strings: dict, icons: dict, key: str) -> None:
    """Each VPN client diagnostic has translated naming and an icon."""
    assert "{tunnel_name}" in strings["entity"]["sensor"][key]["name"]
    assert "default" in icons["entity"]["sensor"][key]
...
def test_vpn_connected_translation_has_tunnel_placeholder(strings: dict) -> None:
    """vpn_connected uses the tunnel name without repeating its state."""
    name = strings["entity"]["binary_sensor"]["vpn_connected"]["name"]
    assert name == "{tunnel_name}"


def test_vpn_peer_connected_translation_uses_peer_name_only(strings: dict) -> None:
    """vpn_peer_connected leaves the connection state to the binary sensor."""
    name = strings["entity"]["binary_sensor"]["vpn_peer_connected"]["name"]
    assert name == "{peer_name}"
...
def test_vpn_listen_port_icon(icons: dict) -> None:
    ...
    assert (
        "mdi:server-network" in icons["entity"]["sensor"]["vpn_listen_port"]["default"]
    )
