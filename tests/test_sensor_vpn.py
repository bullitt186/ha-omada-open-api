...
Server/client schemas are verified against native WireGuard test tunnels on a
Fusion gateway. Sensors still handle missing optional fields gracefully.
"""
...
def test_vpn_client_sensor_description_keys() -> None:
    """VPN clients expose their connection's own peer and traffic data."""
    keys = {description.key for description in VPN_CLIENT_SENSORS}
...
async def test_vpn_client_binary_sensor_uses_tunnel_status(
    hass: HomeAssistant,
) -> None:
    """A connected VPN client has no remote-peer count to aggregate."""
    client_tunnel = {
        "id": "19406080",
        "vpnId": "client-config-id",
        "name": "Remote gateway",
        "status": 1,
        "connectedNum": 0,
        "totalRemoteNum": 0,
    }
    data = _build_vpn_coordinator_data(client=[client_tunnel])
    coord = _make_coordinator(hass, data)
    desc = next(d for d in VPN_BINARY_SENSORS if d.key == "vpn_connected")
    sensor = OmadaVpnBinarySensor(
        coordinator=coord,
        ...
        gateway_mac=GW_MAC,
        vpn_type="client",
        tunnel_id="client-config-id",
        tunnel_name="Remote gateway",
    )
